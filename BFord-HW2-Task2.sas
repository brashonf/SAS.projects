...
proc contents data=data610.s_pml_donor_hw_v3; run;
proc print data= data610.s_pm1_donor_hw_v3 (firstobs=1 obs=10); run;
...
proc mi data=data610.s_pm1_donor_hw_v3 nimpute=1 seed=12345 out=wealth_pmm_imp; 
	fcs regpmm(wealth_rating_mi= wealth_rating_cat_mi MEDIAN_HOME_VALUE PEP_STAR  PER_CAPITA_INCOME);
...
proc mi data=data610.s_pm1_donor_hw_v3 nimpute=1 seed=12345 out=wealth_rating_pmm_imp; 
	fcs regpmm(wealth_rating_mi= wealth_rating median_home_value pep_star per_capita_income);
	var wealth_rating_mi wealth_rating median_home_value pep_star per_capita_income; 
run; 
